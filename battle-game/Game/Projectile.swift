...
enum ProjectileFactory {
    /// Glowing bolt, 24x7pt, oriented along +x (scene sets zRotation).
    static func makeBolt() -> SKShapeNode {
        let bolt = SKShapeNode(ellipseOf: CGSize(width: 24, height: 7))
        bolt.fillColor = SKColor(red: 1, green: 0.87, blue: 0.35, alpha: 1)
        bolt.strokeColor = SKColor(red: 1, green: 0.55, blue: 0.15, alpha: 1)
        bolt.lineWidth = 2
        bolt.zPosition = 15
        let glow = SKShapeNode(ellipseOf: CGSize(width: 36, height: 18))
